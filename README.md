...
| **文章分享** | 微博 / Twitter / 复制链接 | `src/components/ShareButtons.astro` |
...
| **热门文章** | 排行展示 | `src/components/PopularPosts.astro` |
...
侧边栏标签按使用频率自动缩放，高频标签更大更显眼。位于 `src/components/widget/Tags.astro:16`。

### 🔥 热门文章排行

提供排行展示组件，可按需放置在侧边栏或首页。位于 `src/components/PopularPosts.astro:1`。
